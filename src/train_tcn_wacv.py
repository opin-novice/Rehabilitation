...
train_tcn_wacv.py
=================
Temporal Convolutional Network trained on the EXACT SAME subject-disjoint folds as EGRU/PCT.

This is a generic non-spatial, non-equivariant temporal baseline to isolate whether EGRU's
viewpoint invariance (Block 3) is architecture-specific or generalizes to other non-equivariant
models.

TCN operates on flattened joint features [B, J*C, T] → 4 dilated causal convolutions → mean pool.
No graph structure, no rotation invariance by construction. Expected to fail Block 3 similarly to PCT
(degradation on the order of 10 MAD), but differently (TCN has no fixed-frame requirement).

Run:
  python src/train_tcn_wacv.py --fold 0
  python src/train_tcn_wacv.py --cv
"""
...
def evaluate(model, samples, device, batch_size=8):
    """Evaluate TCN on KIMORE samples (no resampling to fixed grid)."""
    model.eval()
    preds, trues = [], []
    for i in range(0, len(samples), batch_size):
        chunk = samples[i: i + batch_size]
        # TCN input contract: [B, T, J, C]
...
def train_fold(train_s, val_s, test_s, args, device, verbose=True):
    ...
    if verbose:
        print(f"  TCN baseline: {n_par:,} params | seq_len={args.seq_len} "
              f"d_model={args.d_model} num_blocks={args.num_blocks}")
...
def main():
    ...
    ap.add_argument("--d-model", type=int, default=128, help="TCN embedding dimension")
    ...
    print(f"\n{'='*78}")
    print(f"TCN (Temporal Convolutional Network) — WACV baseline training")
    print(f"{'='*78}")
    ...
    for f in fold_ids:
        ...
        # Save checkpoint
        ckpt_path = os.path.join(args.ckpt, f"tcn_wacv_s{args.seed}_f{f}.pt")
    ...
    results_path = os.path.join(args.ckpt, f"tcn_wacv_s{args.seed}_results.json")
