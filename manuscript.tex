...
\textbf{Naive kinematic baseline:} For each sequence, we compute total joint path length (sum of Euclidean distances across frames) and mean joint speed, then fit a logistic regression on these two features. The AUROC of this baseline is always computed on the same mapped sequences as the trained models.
...
\begin{table}[t]
\centering
\caption{Zero-shot cross-sensor AUROC. All values are means across 77 fold models. The naive kinematic baseline uses joint path length and mean speed on the same sequences. Degeneracy status: D = degenerate ($\text{pred\_SD} < 0.10$).}
\label{tab:zeroshot}
\begin{tabular}{lcc}
\toprule
...
Every learned condition performs at chance (AUROC 0.51--0.53) on both corpora. The naive kinematic baseline beats every SSL condition on both corpora. Rank Spearman correlation is $|\rho| < 0.03$ across all conditions, confirming no ordinal transfer.
