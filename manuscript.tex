...
\begin{table}[t]
\centering
\caption{Zero-shot cross-sensor AUROC for the IRDS-only pretraining pool ($\sim$1,000 sequences). All values are means across 77 fold models. The naive kinematic baseline uses joint path length and mean speed on the same sequences. Degeneracy status: D = degenerate ($\text{pred\_SD} < 0.10$). The transductive all-corpora pool ($\sim$5,000 sequences, including REHAB246 and UI-PRMD) was not evaluated zero-shot because the within-domain pool-size ablation (Table~\ref{tab:pool}) shows that more unlabeled data does not improve performance.}
\label{tab:zeroshot}
\begin{tabular}{lcc}
\toprule
Condition & REHAB246 (OptiTrack) & UI-PRMD (Kinect) \\
\midrule
A. Scratch                         & 0.516         & 0.524 (D) \\
B. Contrastive LP                  & 0.516         & 0.518 (D) \\
C. Contrastive FT                  & 0.515         & 0.514 \\
D. Masked LP                       & 0.527         & 0.512 (D) \\
E. Masked FT                       & 0.519         & 0.514 (D) \\
\midrule
