...
\begin{table}[t]
\centering
\caption{Zero-shot cross-sensor AUROC. IRDS-only values are mean $\pm$ std across 77 fold models; all-corpora values are from a single model trained on all KIMORE data. The naive kinematic baseline is direct AUROC of path-length and speed features (no model trained). Degeneracy status: D = degenerate ($\text{pred\_SD} < 0.10$). CORAL fits a domain-aligned logistic regression on scratch TCN features from KIMORE.}
\label{tab:zeroshot}
\begin{tabular}{lccc}
\toprule
...
\midrule
A. Scratch                         & $0.516 \pm 0.012$        & $0.524 \pm 0.017$ (D) & 0.513 & 0.527 (D) \\
B. Contrastive LP                  & $0.516 \pm 0.011$        & $0.518 \pm 0.015$ (D) & 0.502 & 0.524 \\
C. Contrastive FT                  & $0.515 \pm 0.012$        & $0.514 \pm 0.013$     & 0.503 & 0.509 \\
D. Masked LP                       & $0.527 \pm 0.011$        & $0.512 \pm 0.009$ (D) & 0.507 & 0.507 \\
E. Masked FT                       & $0.519 \pm 0.012$        & $0.514 \pm 0.009$ (D) & 0.523 & 0.524 \\
\midrule
