...
The barrier is sensor-level domain shift, not representation quality: the null is stable across pretext tasks, pool sizes, evaluation protocols, and two independent test corpora.
...
\begin{table}[t]
\centering
\caption{KIMORE 77-fold true LOSO. Mean Spearman $\rho$ with 95\% bootstrap CI. Holm-Bonferroni adjusted $p$ versus scratch from paired Wilcoxon on absolute error ($N=380$).}
\label{tab:loso}
\begin{tabular}{lccc}
\toprule
Condition & Mean $\rho$ & 95\% CI & $p_{\text{adj}}$ vs.\ scratch \\
\midrule
A. Scratch          & \textbf{0.836} & [0.785, 0.867] & --- \\
E. Masked FT        & 0.823          & [0.773, 0.854] & 0.318 \\
C. Contrastive FT   & 0.816          & [0.762, 0.851] & 0.318 \\
B. Contrastive LP   & 0.689          & [0.617, 0.738] & $3.4\times10^{-14}$ \\
D. Masked LP        & 0.679          & [0.612, 0.727] & $7.3\times10^{-18}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textit{ACM Trans. Graph.}, vol.~34, no.~6, pp.~1--16, 2015.
