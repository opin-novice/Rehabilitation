...
(1) zero-shot AUROC is at chance (0.51--0.53) on both corpora; a naive path-length-plus-speed baseline nominally leads on REHAB246 (AUROC = 0.55) and is interleaved with the learned conditions on UI-PRMD (0.53 vs.\ 0.52--0.53), but under a bootstrap over the ten target subjects the learned models are statistically indistinguishable from chance and the naive baseline's advantage over them is largely not significant;
...
\begin{enumerate}
    \item \textbf{A robust negative result:} SSL pretraining on unlabeled skeletons does not enable zero-shot cross-sensor transfer. Across two pretext tasks (contrastive and masked-motion), two pool sizes ($\sim$1,000 and $\sim$5,000 unlabeled sequences), two independent labeled test corpora (REHAB246, OptiTrack; UI-PRMD, Kinect), and two evaluation protocols (5-fold and 77-fold leave-one-subject-out), every condition scores at chance (AUROC 0.51--0.53). A simple naive kinematic baseline nominally leads on REHAB246 (AUROC 0.55) and ties the learned conditions on UI-PRMD (0.53), but a bootstrap over the ten target subjects shows the learned models are statistically indistinguishable from chance and the naive baseline's advantage over them is largely not significant (Table~\ref{tab:auroc-sig})---no method, learned or naive, achieves subject-level-meaningful cross-sensor discrimination.
...
\emph{Geometry note.} UI-PRMD distributes its Kinect skeletons as \texttt{Positions} files that are \emph{not} world coordinates: they are per-frame parent-relative bone offsets, with joint~0 carrying the absolute root translation. Read literally --- as our own earlier pipeline did --- 57.3\% of the resulting coordinates are constant over time (REHAB246: 0.0\%), so the model sees a near-rigid skeleton dragged along a root trajectory. World positions must be recovered by forward kinematics against the companion \texttt{Angles} files, using the convention published with the dataset's own visualization code. We validated the reconstruction against the corpus's Vicon recordings, which are true marker coordinates: reconstructed body height tracks Vicon height at $r = 0.78$ (deep squat) and a neutral standing frame reconstructs to within 3\,cm of the Vicon head marker. Every UI-PRMD number in this paper is computed on the forward-kinematics geometry. The REHAB246 column is unaffected throughout and is unchanged, which we use as a control: re-running the subject-clustered bootstrap reproduces its published values exactly.
...
\midrule
Scratch         & 0.516 (.003)  & 0.529 (.020) D & 0.515 & 0.527 D \\
Contrast.~LP    & 0.516 (.002)  & 0.520 (.015) D & 0.515 & 0.530 D \\
Contrast.~FT    & 0.515 (.003)  & 0.532 (.014) D & 0.515 & 0.519 D \\
Masked~LP       & 0.527 (.002)  & 0.527 (.018) D & 0.513 D & 0.516 D \\
Masked~FT       & 0.519 (.003)  & 0.525 (.017) D & 0.518 & 0.509 D \\
\midrule
CORAL           & 0.522 (.003)  & 0.516 (.018)   & ---   & --- \\
\midrule
Naive kinem.    & \textbf{0.554} & \textbf{0.527} & ---   & --- \\
\bottomrule
\end{tabular}
\end{table}

All the learned conditions perform at chance (AUROC 0.51--0.53) on both corpora. At the pool-sample level the naive kinematic baseline nominally outperforms every SSL condition on REHAB246; on UI-PRMD it does not, sitting inside the learned spread (naive 0.527 against 0.520--0.532), so on that corpus there is no consistent nominal ordering to begin with. In either case the ranking is invalid under subject-clustered bootstrap (Table~\ref{tab:auroc-sig}). All the rank Spearman correlations between all the conditions are $|\rho| < 0.03$, confirming the lack of ordinal transfer. Per-fold standard deviations are 0.009--0.017 (95\% CI $\approx \pm 0.003$), confirming the stability of the null hypothesis rather than a statistical fluctuation. AUPRC values (0.51--0.54) replicate the AUROC values, ruling out class imbalance artifacts. CORAL domain adaptation baseline --- aligning second-order statistics of scratch TCN features from the labeled KIMORE data before logistic regression --- is also at chance (AUROC 0.522 and 0.516), confirming that even the source-labeled data cannot alleviate the domain shift via simple feature alignment.

\textbf{Subject-level significance (ten-subject bootstrap).} Because each target corpus has ten subjects only, the repetition-level intervals overestimate the effective sample size. So we re-analyzed all the conditions as well as the naive baseline under a bootstrap sampling subjects rather than repetitions (2000 resamples). Orientation of each score is fixed under the full sample once so that the resampling cannot boost AUROC via taking $\max(\text{AUROC},\,1-\text{AUROC})$ per draw (Table~\ref{tab:auroc-sig}). Under this honest assessment against the subject-level variance, there is no learned model whose AUROC is distinguishable from 0.50 on REHAB246 (chance lies within 95\% subject confidence interval for every model) and only trivial effects exceed chance on UI-PRMD (Contrastive-FT 0.537, Masked-LP 0.525; both are within 0.04 of chance). The naive baseline, however, exceeds chance on both corpora (REHAB246 0.554, CI [0.506, 0.605], $p = 0.035$; UI-PRMD 0.527, $p < 0.001$), yet its superiority over the learned models is not statistically significant for \emph{any} of the ten model $\times$ corpus comparisons ($p = 0.15$--$0.86$). Hence, the ordering of Table~\ref{tab:zeroshot} does not survive the subject-level uncertainty: at the subject level, neither the learned models nor the naive baseline can achieve a reliable cross-sensor discrimination.

Which particular conditions clear the chance threshold on UI-PRMD is not stable, and we do not interpret it. On the degenerate geometry of our earlier build it was Scratch and Contrastive-LP; on corrected geometry it is Contrastive-FT and Masked-LP. The magnitudes are trivial in both cases and no condition is separable from the naive baseline in either, which is the claim we rely on.
...
\midrule
\multirow{6}{*}{UI-PRMD}
 & Scratch        & 0.510 & [0.489, 0.536] & 0.305 & $+0.015$ & 0.319 \\
 & Contrastive LP & 0.514 & [0.480, 0.553] & 0.451 & $+0.012$ & 0.601 \\
 & Contrastive FT & 0.537 & [0.518, 0.561] & $<$0.001 & $-0.011$ & 0.429 \\
 & Masked LP      & 0.525 & [0.509, 0.542] & 0.002 & $+0.002$ & 0.861 \\
 & Masked FT      & 0.513 & [0.497, 0.527] & 0.112 & $+0.015$ & 0.150 \\
 & Naive baseline & \textbf{0.527} & [0.515, 0.543] & \textbf{$<$0.001} & --- & --- \\
\bottomrule
\end{tabular}
\end{table*}

The all-corpora column in Table~\ref{tab:zeroshot} shows the transductive upper bound: SSL pretrained on a pool that comprises the unlabeled target corpora. The results are identical --- all the conditions are at chance (AUROC 0.509--0.530) --- confirming that even the direct access to unlabeled target distributions during pretraining does not increase the cross-sensor transfer. This column is not a re-scoring of the published encoders: UI-PRMD supplies 2{,}000 of the pool's 4{,}057 unlabeled sequences, so the encoders themselves were re-pretrained from scratch on the corrected geometry (both pretexts, 300 epochs) and the five conditions re-fine-tuned before re-evaluation. The scratch condition, which uses no pretrained encoder, reproduces its previous UI-PRMD value to three decimals (0.527), bounding the retraining noise in this column. This is consistent with the within-domain pool-size ablation (Table~\ref{tab:pool}), which failed to close the gap when increasing the size of the unlabeled pool fourfold.

On REHAB246, all the IRDS-only conditions are non-degenerate ($\text{pred\_SD} > 0.10$); the chance-level AUROC is therefore a real transfer failure rather than a collapse of the predictor (one all-corpora condition, Masked~LP, is borderline at $0.080$). On UI-PRMD, \emph{all five} conditions are degenerate ($\text{pred\_SD} = 0.006$--$0.048$, all $< 0.10$), which indicates that the models collapse to near-constant outputs on this corpus. Degeneracy on UI-PRMD is thus more complete than on REHAB246, not less: there is no non-degenerate condition whose chance-level AUROC could be read as a genuine transfer attempt.

\textbf{Per-exercise analysis:} On REHAB246 (six exercises), per-exercise AUROCs of SSL range from 0.53 to 0.64, depending on the pretext task, with minor difference between pretexts. The naive kinematic baseline yields much higher per-exercise AUROCs (0.63--0.87), confirming that simple motion features are exercise-discriminative while SSL encoders fail to use them for cross-sensor discrimination. On UI-PRMD (ten exercises), SSL per-exercise AUROCs vary from 0.51 to 0.79, with no stable ordering across exercises: the per-exercise pattern is not reproducible across preprocessing choices. On our earlier degenerate build the apparent maxima sat on ex1 (deep squat, up to 0.86) and ex4, which invited a reading in terms of well-isolated movements; on corrected forward-kinematics geometry ex1 falls to 0.54--0.61 while ex5 rises to 0.79. We therefore report the per-exercise spread only as a check that no single exercise carries the aggregate result, and draw no conclusion about which movement types transfer. Naive and learned per-exercise values are interleaved --- the naive baseline is the higher of the two on four of the ten exercises and the lower on six --- so we do not claim per-exercise dominance for either; what the breakdown does show is that the aggregate null is not an average over a few discrepant exercises.
...
\textbf{Few-shot calibration:} We probed whether 1, 5, 10, or 20 labeled target-sensor samples are needed to calibrate a logistic regression on frozen TCN features. For all the conditions and both corpora, few-shot AUROC is still at chance (0.51--0.54 even at $n = 20$), with $n = 1$ failing to yield a two-class evaluation. On REHAB246 the 20-shot regime remains below the naive baseline ($0.528$ vs.\ $0.554$); on UI-PRMD it reaches $0.542$ against a naive baseline of $0.527$, a nominal excess of $0.015$ that the subject-clustered bootstrap cannot separate from chance. Either way, a modest handful of labelled target examples does not compensate for the cross-sensor gap.
...
\textbf{Naive-baseline sensitivity (joint padding and normalization).} Because the naive kinematic baseline is based on the harmonized 25-joint sequence, we have confirmed that it is not an artefact of joint padding or normalization (Table~\ref{tab:naive-sens}). Restriction of the features to truly distinct joints excluding three padded slots of UI-PRMD and four duplicated permutation targets (thumb${=}$wrist) of REHAB246 leaves the baseline essentially the same (REHAB246 $0.554 \to 0.548$; UI-PRMD identical at $0.527$, since a difference-based path/speed feature of an all-zeros joint contributes exactly zero). Under strict per-sequence z-scoring the two corpora behave in opposite directions: REHAB246 drops to chance ($0.510$), so the edge of the naive baseline there lies exactly in the global scale/speed cue whose loss we argued affects cross-sensor transfer negatively (Section~\ref{sec:datasets}); UI-PRMD instead \emph{rises} to $0.609$, its strongest value anywhere in this study. Removing per-sequence scale therefore does not remove UI-PRMD's discriminative kinematic signal --- it sharpens it, presumably because the correct/incorrect contrast there is one of movement \emph{shape} rather than of overall magnitude, and per-sequence normalization suppresses between-subject amplitude variation that otherwise masks it. Crucially, under either normalization the naive baseline is not below the learned models, so the learned conditions are not disadvantaged by the comparison.
...
To test whether the null hypothesis is specific to the TCN backbone, the coordinate input, or the weak (CORAL) adaptation baseline, we repeated the zero-shot evaluation under several stronger settings (Table~\ref{tab:robustness-c}). (i) A spatial-prior \textbf{ST-GCN}~\cite{yan2018stgcn} backbone, trained under the identical 77-fold LOSO, remains at chance (REHAB246 $0.522\pm0.008$; UI-PRMD $0.517\pm0.010$) and below the naive baseline; crucially, 3-way sensor-identity probe on ST-GCN embeddings remains \emph{perfect} (balanced accuracy $1.00$ vs.\ chance $0.33$), confirming that sensor entanglement is not a TCN artifact but architecture independent. (ii) Bone-length-preserving \textbf{relative-joint-vector input} (each joint expressed relative to its skeletal parent; translation-invariant), used as the native model input rather than the post-hoc one, does not help (REHAB246 $0.534\pm0.017$; UI-PRMD $0.530\pm0.022$) --- nominally the highest learned AUROC, but not evidence of transfer: the direction-agnostic $\max(\text{AUROC}, 1-\text{AUROC})$ metric is biased upwards above 0.5 by construction, and the corresponding rank-Spearman correlations ($0.05$ and $0.02$) confirm no ordinal transfer. (iii) Three stronger domain-adaptation and domain-generalization baselines --- parameter-free \textbf{AdaBN}~\cite{li2018adabn} (target BatchNorm re-estimation on the ST-GCN), \textbf{DANN}~\cite{ganin2016dann} (gradient-reversal domain-adversarial training), and \textbf{SWAD}~\cite{cha2021swad} (flat-minima domain generalization via dense stochastic weight averaging over the training tail) remain at chance (AdaBN $0.519$/$0.511$; DANN $0.529$/$0.521$; SWAD $0.507$/$0.534$, both non-degenerate). DANN is retrained from scratch at evaluation time and its run-to-run spread is appreciable --- a repeat run moved the unaffected REHAB246 value by $0.026$ --- so its UI-PRMD shift should not be read as a geometry effect. (iv) Finally, replacement of the global train-fit standardization with strict \textbf{per-sequence z-scoring} --- the normalization suspected as potentially discarding transfer-relevant scale does not change the result, leaving it at chance (REHAB246 $0.539\pm0.023$; UI-PRMD $0.549\pm0.035$), so that the null hypothesis does not depend on the normalization scheme; note the like-for-like comparison here is against the naive baseline computed under the same normalization ($0.510$ / $0.609$), which the learned models remain below.

Across every architecture, input representation, normalization, and adaptation method tested, no combination achieves cross-sensor discrimination distinguishable from chance, and the sensor identity signal persists. On UI-PRMD two settings now sit nominally above the raw-coordinate naive baseline of $0.527$ --- SWAD ($0.534$) and per-sequence z-scoring ($0.549$, though below its like-for-like $0.609$ baseline) --- by margins of $\le 0.02$ that are smaller than the DANN run-to-run spread above and that the subject-clustered bootstrap (Table~\ref{tab:auroc-sig}) cannot separate from chance. We therefore no longer claim that \emph{no} setting reaches the naive baseline on UI-PRMD; we claim the weaker and better-supported statement that none of them attains a subject-level-meaningful margin over it.

\begin{table}[t]
\centering
\small
\setlength{\tabcolsep}{3pt}
\caption{Robustness of the zero-shot null to backbone, input representation, and domain adaptation/generalization. AUROC (mean $\pm$ std across 77 folds where applicable). D = degenerate ($\text{pred\_SD}<0.10$). Naive kinematic baseline: 0.554 (REHAB246) / 0.527 (UI-PRMD). UI-PRMD values are recomputed on forward-kinematics geometry (Sec.~\ref{sec:datasets} footnote); the REHAB246 column is unaffected and unchanged. The per-sequence z-score row must be read against the naive baseline computed under the \emph{same} normalization (0.510 REHAB246 / 0.609 UI-PRMD), not against the raw-coordinate baseline.}
\label{tab:robustness-c}
\begin{tabular}{@{}lcc@{}}
\toprule
Setting & REHAB246 & UI-PRMD \\
\midrule
ST-GCN backbone         & $0.522{\pm}0.008$ & $0.517{\pm}0.010$ (D) \\
\quad + AdaBN           & $0.519{\pm}0.009$ & $0.511{\pm}0.010$ \\
Rel.-joint input        & $0.534{\pm}0.017$ & $0.530{\pm}0.022$ \\
Per-seq.\ z-score       & $0.539{\pm}0.023$ & $0.549{\pm}0.035$ \\
DANN                    & $0.529$           & $0.521$ \\
SWAD (flat-minima DG)   & $0.507$           & $0.534$ \\
\midrule
Naive kinematic         & \textbf{0.554}    & \textbf{0.527} \\
\bottomrule
\end{tabular}
\end{table}
