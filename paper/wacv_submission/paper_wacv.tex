...
a relative MAD reduction with no recoverable absolute baseline. Lacking their preprocessed data and a
subject-disjoint rerun, we treat these as measured under a different protocol, not as an accuracy
ceiling.
...
$33.96$ of fifty); TCN $13.17$~MAD; ST-GCN $9.18$~MAD. This pattern holds across three independently-designed non-equivariant architectures (temporal-conv,
...
fairness objection rather than assuming it: granting the hand-crafted features an
...
joints, and a viewpoint invariance empirically exact for arbitrary $SO(3)$, not merely the azimuth
subgroup a clinician sweeps (worst canonical-coordinate shift $3.7\times10^{-11}$ over $32$ random
rotations). This is the strong form of the baseline: its axes are sign-disambiguated by a deterministic
rule (each oriented so its dominant joint projection is positive) and the frame forced right-handed, so
the discrete $\pm$ ambiguity is resolved before any number below is measured.
...
On KIMORE $\gamma$ is small far more often than not --- $18\%$ of frames within $0.05$ of degeneracy and
$45\%$ within $0.10$, a standing body being nearly axially symmetric --- so amplification is the common
case, not a corner. Injecting camera-frame noise at $2\%$ of body scale and comparing the frame
...
frozen nodes. The two models \emph{cross} between $4$ and $8$ frozen joints; full curve and fold-level
