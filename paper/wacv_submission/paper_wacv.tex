...
%  PRIMARY SOURCE for the WACV submission. The 17pp IEEEtran twins
%  (paper.tex / paper_overleaf.tex) are FROZEN as the arXiv extended version.
%  Condensation map: WACV_CUT_LIST.md
...
as a \emph{theorem}, certified to the fp64 roundoff floor ($\sim\!10^{-16}$ on KIMORE,
$\sim\!10^{-9}$ on the \emph{real} $45^\circ$ NTU~RGB+D Cross-View camera displacement). On clean frontal
...
and validated on real NTU~RGB+D Cross-View ($5.8\times10^{-9}$, fp64), against a baseline that crosses
...
passed, each re-runnable and refutable from the released code: representation orthogonality
...
All experiments are pooled, subject-disjoint, over 3 seeds. \emph{Protocol audit}
...
is inside the $0.33$~MAD \emph{fixed-configuration} floor (seed-to-seed std $0.48$, supp.\
\cref{tab:seed}). $\Delta$ vs.\ floor is the SAME paired subject-cluster bootstrap statistic used in
...
$9\times10^{-6}$~MAD; InvariantGRU exactly $0$), while PCT crosses the mean-predictor floor between
$30^\circ$ and $45^\circ$ (worst degradation $9.42$~MAD). Rotation augmentation flattens PCT's aggregate
curve but leaves a $3.03$~MAD \emph{per-sequence} score shift --- the difference between a guarantee and
a fit. The guarantee is not synthetic: on NTU~RGB+D~$60$ Cross-View, whose test camera is $45^\circ$
displaced, the identical encoder trained \emph{without} rotation augmentation attains exact cross-view
logit invariance ($5.8\times10^{-9}$, fp64).

\begin{figure}[t]\centering
  \includegraphics[width=\linewidth]{fig_viewpoint}
  \caption{MAD vs.\ camera azimuth. Certified-invariant models are flat by theorem; the transformer
  degrades through the mean-predictor floor. Augmentation flattens the mean but not the per-sequence
  swing ($3.03$~MAD).}
  \label{fig:viewpoint}
\end{figure}
...
construction (\cref{prop:invariance}), certified to roundoff, and the \emph{same} certificate --- with no
retraining --- holds on NTU~RGB+D Cross-View, a corpus with a genuinely displaced $45^\circ$ camera
($5.8\times10^{-9}$). A frame estimator offers neither the worst-case bound nor that transfer. We do
...
(median), an honest time-to-first-\emph{reliable}-score for real-time use, and preserves invariance to
$1.4\times10^{-8}$, at a $3.2$-point NTU accuracy cost vs.\ the bidirectional model (NTU-60 X-View SOTA
...
\midrule
\multicolumn{4}{l}{Streaming: $8.9$~ms/frame ($112$~fps); invariance $1.4\times10^{-8}$;} \\
...
% =====================================================================
% Bibliography reused verbatim from the journal source (paper.tex).
