...
matches us in aggregate accuracy while still swinging an individual patient's score by $3.03$ MAD
...
its \emph{per-sequence} degradation is $3.03$~MAD, individual scores swinging three points of fifty as
the camera moves, the errors merely cancelling in the mean. Ours move by $9\times10^{-6}$. We therefore
...
azimuth the certified-invariant models are flat to machine precision (EGRU worst-case
$9\times10^{-6}$~MAD; InvariantGRU exactly $0$; hand-crafted invariant features Ridge at the fp64
...
$30^\circ$ and $45^\circ$ (worst degradation $9.42$~MAD); TCN worst-case $13.17$~MAD; ST-GCN worst-case
...
PCT-specific. Rotation augmentation flattens PCT's aggregate curve but leaves a $3.03$~MAD \emph{per-sequence}
score shift --- the difference between a guarantee and a fit. The certificate is over Haar-random
...
\begin{figure}[t]\centering
  ...
  Augmentation flattens PCT's aggregate but not per-sequence swing ($3.03$~MAD). Azimuth here is a
...
Movement $1.9$--$4.8$ per subscale), putting our $3.03$-MAD (of $50$) per-sequence swing in the same
order of magnitude as an established change-significance threshold on cognate instruments, and our int8
and streaming score shifts ($0.05$ and lower, \cref{sec:edge}) one to two orders of magnitude below it.
...
worst per-sequence degradation is $3.03$~MAD against our $9\times10^{-6}$, at $7.4\times$ more
parameters. Behind the \emph{identical} invariant cut, a lighter $E(n)$-equivariant (EGNN) encoder ties
...
\begin{table}[t]\centering\small
\setlength{\tabcolsep}{4pt}
\caption{Does each model clear the floor ($8.31$~MAD) under each stress? Last column: worst
per-sequence score shift under any camera rotation. EGNN shares the identical cut (viewpoint exact
...
\toprule
Model & Params & Clean & $90^\circ$ & $2$ dead & Worst degr. \\
\midrule
...
\begin{figure}[t]\centering
  ...
  \caption{\textbf{The axis that separates the survivors.} (a)~Worst-case per-patient score shift under
  camera rotation (log scale): the certified models (EGRU, EGNN) hold a patient's score to
  $\sim\!10^{-5}$~MAD, six orders of magnitude below the augmented and unaugmented transformers.
...
and to causal streaming. We recommend the field report a mean-predictor floor and per-sequence
degradation alongside aggregate error, and evaluate off the frontal manifold --- camera relocation and
