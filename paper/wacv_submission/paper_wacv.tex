...
to learn --- though we stress-tested the epoch budget, not augmentation strength or schedule, and a
differently tuned augmentation may narrow the gap. The certificate is over Haar-random $SO(3)$, so NTU~RGB+D~$60$ Cross-View's $45^\circ$
...
$1$, the identical encoder reaches $82.98\%$ Top-1 ($T{=}100$, single-clip, one seed). We report no
ST-GCN control beside it: our available runs differ in sampling, view-normalisation and optimiser, so
the pair would not be a comparison. That same single-clip protocol is why this sits below the
$\sim\!88\%$ reported for this split.
...
\textbf{When the worst case is the deployment case.} A home device is set up by the patient, not a
technician, and left unattended --- the camera sits wherever a phone stand or laptop lid lands, and
the consumer depth sensor is the component most likely to lose a joint (a hand behind the back, a node
that freezes mid-repetition). Neither cheaper alternative
...
provably safe against both at once. That is the niche: not that InvariantGRU or Canon-PCA$+$PCT are
worse in a supervised clinic with a fixed rig, but that neither offers a bound surviving unsupervised
deployment.
