...
Every round-4 concern is addressed with analysis on the **already-trained** models (no
retraining), and the central claim is reframed to what the subject-level statistics
actually support. Line references are to the revised `manuscript.tex`; raw numbers are in
...
| **C8** | DA/DG treatment too shallow | Justified CORAL/AdaBN/DANN as **one representative per family** (moment-matching / normalization / adversarial), and named the untested complementary paradigms — IRM, SWAD (domain generalization) and TENT (test-time adaptation) — while noting the perfect probe locates the failure in the representation itself. +2 references (IRM, SWAD). | §VI Limitations (Ninth) + bibliography |
...
and `src/reviewer_round4_embed.py`. Manuscript recompiles clean (10 pp, no undefined
